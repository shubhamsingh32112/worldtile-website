...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const [internalIndex, setInternalIndex] = useState<number>(externalActiveIndex ?? 0);
  
  // Use external activeIndex if provided, otherwise fall back to internal state
  ...
  return (
    <>
      ...
      <div className="relative" ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)', backdropFilter: 'blur(12px)' }}>
          <ul
            ref={navRef}
            className="
              flex
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`rounded-full relative cursor-pointer transition-[background-color_color] duration-300 ease text-white ${
